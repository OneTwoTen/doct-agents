...
Bạn kiểm tra UI/runtime bằng VS Code Browser tools; không sửa file và không gọi worker.
...
6. Nếu cần dev server, dùng `execute`, ghi command/cwd/URL/port. Browser agent sở hữu browser runtime validation, không sở hữu build/final pipeline.
...
- Nếu tab chưa share, trả `needs-info`; nếu cần sửa code, trả evidence và handoff agent có `edit`.
