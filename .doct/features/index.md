# Danh mục tính năng

| Feature | Status | Related spec | Last changed |
| --- | --- | --- | --- |
| Orchestrator-only routing | stable | historical | 2026-07-30 |
| FAST_FIX workflow | stable | historical | 2026-07-30 |
| Browser-driven implementation loop | stable | historical/main | 2026-08-04 |
| LONG_RUNNING workflow | experimental | `.doct/specs/doct-spec-workspace/` | 2026-08-04 |
| Documentation impact lifecycle | stable | historical | 2026-07-30 |
| Executor-neutral spec workspace | experimental | `.doct/specs/doct-spec-workspace/` | 2026-08-04 |
| Feature registry | experimental | `.doct/specs/doct-spec-workspace/` | 2026-08-04 |
| Evidence-backed task checklist | experimental | `.doct/specs/doct-spec-workspace/` | 2026-08-04 |
| Validation ownership | stable | historical | 2026-07-31 |

## Ý nghĩa status

- `planned`: đã được chấp thuận nhưng chưa triển khai.
- `in-progress`: implementation chưa hoàn tất.
- `experimental`: đã có implementation contract nhưng final validation/stability evidence còn thiếu.
- `stable`: capability hiện tại đã được validate.
- `deprecated`: chỉ giữ để tương thích và dự kiến loại bỏ.
- `removed`: không còn được hỗ trợ.

Feature records trong thư mục này mô tả current-state behavior. Specs dưới `.doct/specs/` mô tả change history và execution state.
